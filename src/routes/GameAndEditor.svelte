<script lang="ts">
  ...
  import game from "$assets/images/home/game.svg";
  import player from "$assets/images/home/player.svg";
</script>
...
<div class="relative md:flex-1 lg:flex-none">
  ...
  >
    ...
    />
    <img
      class="absolute left-0 top-0 h-full w-full origin-center scale-50 saturate-0"
      id="player"
      width="32"
      height="96"
      src={player}
...
<style lang="postcss">
  * {
  ...
  #player-blur {
    animation-name: player-blur;
  }

  @keyframes editor {
    100% {
      transform: translate(8%, 0%) rotate(5deg) scale(80%);
    }
  }

  @keyframes game {
    100% {
      transform: translate(-6%, 0%) rotate(-6deg) scale(83%);
    }
  }
  ...
  @keyframes player {
    100% {
      filter: none;
      transform: translate(-1%, -1%) rotate(-5deg) scale(55%);
    }
  }

  @keyframes player-blur {
    100% {
      opacity: 50%;
      filter: blur(1rem);
      transform: translate(-1%, -1%) rotate(-5deg) scale(55%);
